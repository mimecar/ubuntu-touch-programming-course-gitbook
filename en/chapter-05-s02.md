...
To use it is necessary this import statement:
...
![Application source tree](chapter-05-s02/01_application_source.png)
...
![Weather Recorder new design](chapter-05-s02/03_application_new_design.png)
...
![Analytic Page](chapter-05-s02/04_chart.png)
...
The previous section have already shown the new chart page added at our application.
Here, we want describe in more details the changes added.
...
After a splitting of that valu (see "listing 05", is created a Javascript Date object to use his native methods to calculate the first and last days of the chosen month
(this calculation must be done at runtime because we don’t know the user choice).
...
Only a couple of tips about how UbuntuListView works: it receive in input (with the field named "model") the ListModel containing the values to display and a QML component that define the layout used to show them in the page (field "delegate").
The "delegate" component access at the values in the ListModel using the key values used to store them (i.e. "date" and "temp" looking at listing 11).
